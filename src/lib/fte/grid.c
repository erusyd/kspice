...
GRAPH *graph;
{
    ...
    if ((relcy > - graph->grid.xaxis.circular.radius ) 
        ...
        if (zheight < 0)
            zheight = - zheight;
	DrawLine( graph->grid.xaxis.circular.center - zheight,
		  graph->grid.yaxis.circular.center + relcy,

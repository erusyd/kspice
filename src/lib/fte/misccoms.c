...
/* Some C compilers can't handle strings longer than 256 characters */
    ...
    wordlist *wl;
{
    ...
    if (wl == NULL) {
	if (allflag)
	    out_printf(SHORTHELPMSG, cp_program, cp_program);
	else {
...
com_bug(wl)
    wordlist *wl;
{
    fprintf(cp_out, "Send mail to the address %s-bugs@Berkeley.EDU\n",
        ft_nutmeg ? "nutmeg" : "spice");
    return;
}

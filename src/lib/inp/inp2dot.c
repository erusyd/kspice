...
char *word;     /* something to stick a word of input into */
	...
	IFC(newAnalysis,(ckt,which,"distoan",&foo, task))
	...
	(*(ft_sim->newAnalysis)) (ckt,which,"noise",&foo,task);
	INPgetTok(&line,&name,1);
	...
	if (*name != (char)NULL) {
	    length = strlen(name);
	    if (((*name == 'V') || (*name == 'v')) && (length == 1)) {
		parm = INPgetValue(ckt,&line,IF_STRING,tab);
		error = INPapName(ckt,which,foo,"output",parm);

		/* See if an output reference node is specified, ie the */
		/* command of the form V(xxxx), or of the form V(xxxx,yyyy) */

		for (found=0, point=line; (found == 0) && (*point != '\0');
		    found = (*(point++) == ')'));
		if (found) {
		    parm = INPgetValue(ckt,&line,IF_STRING,tab);
		    error = INPapName(ckt,which,foo,"outputref",parm);
		} else {
		    ptemp.sValue = (char*)NULL;
		    error = INPapName(ckt,which,foo,"outputref",&ptemp);
		}
		if (error)
		    current->error = INPerrCat(current->error,INPerror(error));
		parm = INPgetValue(ckt,&line,IF_STRING,tab);
		error = INPapName(ckt,which,foo,"input",parm);
		if (error)
		    current->error = INPerrCat(current->error,INPerror(error));

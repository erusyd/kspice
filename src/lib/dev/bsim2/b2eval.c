 ...
 */
void
BSIM2evaluate(Vds,Vbs,Vgs,here,model,gm,gds,gmb,qg,qb,qd,cgg,cgd,cgs,
        cbg,cbd,cbs,cdg,cdd,cds,Ids,von,vdsat,ckt)

    register CKTcircuit  *ckt;
    register BSIM2model   *model;
    ...
    double *cbd,*cbs,*cdg,*cdd,*cds,*Ids,*von,*vdsat;
 {
    ...
    if (Vbs < model->BSIM2vbb2) Vbs = model->BSIM2vbb2;
    if (Vgs > model->BSIM2vgg2) Vgs = model->BSIM2vgg2;
    if (Vds > model->BSIM2vdd2) Vds = model->BSIM2vdd2;

/* Threshold Voltage. */
    if (Vbs <= 0.0)
    {   Phisb = here->pParam->BSIM2phi - Vbs;
        dPhisb_dVb = -1.0;
        T1s = sqrt(Phisb);
        dT1s_dVb = -0.5 / T1s; 
    }
    else
    {   tmp = here->pParam->BSIM2phi / (here->pParam->BSIM2phi + Vbs);
        Phisb = here->pParam->BSIM2phi * tmp;
        dPhisb_dVb = -tmp * tmp;
        T1s = here->pParam->Phis3 / (here->pParam->BSIM2phi + 0.5 * Vbs);
        dT1s_dVb = -0.5 * T1s * T1s / here->pParam->Phis3;
    }

    Eta = here->pParam->BSIM2eta0 + here->pParam->BSIM2etaB * Vbs;
    Ua = here->pParam->BSIM2ua0 + here->pParam->BSIM2uaB * Vbs;
    Ub = here->pParam->BSIM2ub0 + here->pParam->BSIM2ubB * Vbs;
    U1s = here->pParam->BSIM2u10 + here->pParam->BSIM2u1B * Vbs;

    Vth = here->pParam->BSIM2vfb + here->pParam->BSIM2phi + here->pParam->BSIM2k1
        * T1s - here->pParam->BSIM2k2 * Phisb - Eta * Vds;
    dVth_dVd = -Eta;
    dVth_dVb = here->pParam->BSIM2k1 * dT1s_dVb + here->pParam->BSIM2k2 
	     - here->pParam->BSIM2etaB * Vds;
    ...
    T0 = Gg / T1s;
    tmp1 = 0.5 * T0 * here->pParam->BSIM2k1;
    Aa = 1.0 + tmp1;
    dAa_dVb = (Aa - 1.0) * (dGg_dVb / Gg - dT1s_dVb / T1s);
    Inv_Aa = 1.0 / Aa;

    Vghigh = here->pParam->BSIM2vghigh;
    Vglow = here->pParam->BSIM2vglow;

    if ((Vgst >= Vghigh) || (here->pParam->BSIM2n0 == 0.0))
    {   Vgeff = Vgst;
	dVgeff_dVg = 1.0;
	dVgeff_dVd = -dVth_dVd;
	dVgeff_dVb = -dVth_dVb;
    ...
    {   Vof = here->pParam->BSIM2vof0 + here->pParam->BSIM2vofB * Vbs
	    + here->pParam->BSIM2vofD * Vds;
	n = here->pParam->BSIM2n0 + here->pParam->BSIM2nB / T1s 
	  + here->pParam->BSIM2nD * Vds;
	tmp = 0.5 / (n * model->BSIM2Vtm);

	ExpArg1 = -Vds / model->BSIM2Vtm;
	...
	if (Vgst <= Vglow)
	{
	    ...
	    Vgeff = sqrt(tmp2) * model->BSIM2Vtm * Exp0;
	    T0 = n * model->BSIM2Vtm;
	    dVgeff_dVg = Vgeff * tmp;
	    dVgeff_dVd = dVgeff_dVg * (n / tmp1 * Exp1 - dVth_dVd - Vgst
		       * here->pParam->BSIM2nD / n + T0 * here->pParam->BSIM2vofD);
	    dVgeff_dVb = dVgeff_dVg * (here->pParam->BSIM2vofB * T0
		       - dVth_dVb + here->pParam->BSIM2nB * Vgst
	...
	else
	{
	    ...
	    ExpArg = MAX(ExpArg, -30.0);
	    Exp0 = exp(0.5 * Vof + ExpArg);
	    Vgeff = sqrt(2.0 * Aa * (1.0 - Exp1)) * model->BSIM2Vtm * Exp0;
	    Con1 = Vghigh;
	    Con3 = Vgeff;
	    Con4 = Con3 * tmp;
	    ...
	    dVgeff_dVg = (Coeffb + Vgst * (2.0 * Coeffc + 3.0 * Vgst * Coeffd))
		       * delta;
	    T7 = Con3 * tmp;
	    T8 = dT1s_dVb * here->pParam->BSIM2nB / (T1s * T1s * n);
	    T9 = n * model->BSIM2Vtm;
	    dCon3_dVd = T7 * (n * Exp1 / tmp1 - Vglow * here->pParam->BSIM2nD 
		      / n + T9 * here->pParam->BSIM2vofD);
	    dCon3_dVb = T7 * (T9 * Inv_Aa * dAa_dVb + Vglow * T8
		      + T9 * here->pParam->BSIM2vofB);
	    dCon4_dVd = tmp * dCon3_dVd - T7 * here->pParam->BSIM2nD / n;
    ...
    if (Vgeff > 0.0)
    {
        ...
        dUvert_dVb = T8 * dVgeff_dVb + Vgeff * (here->pParam->BSIM2uaB
	           + Vgeff * here->pParam->BSIM2ubB);
        ...
        dVc_dVb = T8 * dVgeff_dVb + here->pParam->BSIM2u1B * Vgeff * Inv_Aa 
        ...
        Beta0 = here->pParam->BSIM2beta0 + here->pParam->BSIM2beta0B * Vbs;
        Betas = here->pParam->BSIM2betas0 + here->pParam->BSIM2betasB * Vbs;
        Beta2 = here->pParam->BSIM2beta20 + here->pParam->BSIM2beta2B * Vbs
	      + here->pParam->BSIM2beta2G * Vgs;
        Beta3 = here->pParam->BSIM2beta30 + here->pParam->BSIM2beta3B * Vbs
	      + here->pParam->BSIM2beta3G * Vgs;
        Beta4 = here->pParam->BSIM2beta40 + here->pParam->BSIM2beta4B * Vbs
	      + here->pParam->BSIM2beta4G * Vgs;
        Beta1 = Betas - (Beta0 + model->BSIM2vdd * (Beta3 - model->BSIM2vdd 
	      * Beta4)); 
        ...
        T5 = model->BSIM2vdd * tanh;
        dBeta_dVd = Beta3 - 2.0 * Beta4 * Vds + T4 * (Beta2 - T0 * dVdsat_dVd);
        dBeta_dVg = T4 * (here->pParam->BSIM2beta2G * Vds - T0 * dVdsat_dVg) 
	          + here->pParam->BSIM2beta3G * (Vds - T5) 
		  - here->pParam->BSIM2beta4G * (Vds * Vds - model->BSIM2vdd * T5);
        dBeta1_dVb = here->pParam->Arg;
        dBeta_dVb = here->pParam->BSIM2beta0B + dBeta1_dVb * tanh + Vds
	          * (here->pParam->BSIM2beta3B - Vds * here->pParam->BSIM2beta4B)
	          + T4 * (here->pParam->BSIM2beta2B * Vds - T0 * dVdsat_dVb);
	    ...
	    if (Vds <= Vdsat) /* triode region */
	    {
	        T3 = Vds * Inv_Vdsat;
	        T4 = T3 - 1.0;
	        T2 =  1.0 - here->pParam->BSIM2u1D * T4 * T4;
	        U1 =  U1s * T2;
	        Utot = Uvert + U1 * Vds;
	        Utot = MAX(Utot, 0.5);
	        Inv_Utot = 1.0 / Utot;
	        T5 = 2.0 * U1s * here->pParam->BSIM2u1D * Inv_Vdsat * T4;
	        dU1_dVd = T5 * (T3 * dVdsat_dVd - 1.0);
	        dU1_dVg = T5 * T3 * dVdsat_dVg;
	        dU1_dVb = T5 * T3 * dVdsat_dVb + here->pParam->BSIM2u1B * T2;
	      ...
	      if (here->pParam->BSIM2ai0 != 0.0)
	      {
	          Ai = here->pParam->BSIM2ai0 + here->pParam->BSIM2aiB * Vbs;
	          Bi = here->pParam->BSIM2bi0 + here->pParam->BSIM2biB * Vbs;
	          T5 = Bi / (Vds - Vdsat);
	          T5 = MIN(T5, 30.0);
	          T6 = exp(-T5);
	          FR = 1.0 + Ai * T6;
	          ...
	          dFR_dVb = T8 * dVdsat_dVb + T6 * (here->pParam->BSIM2aiB - Ai
		          * here->pParam->BSIM2biB / (Vds - Vdsat));
       ...
       {   T0 = Exp0 * Exp0;
           T1 = Exp1;
           *Ids = Beta * model->BSIM2Vtm * model->BSIM2Vtm * T0 * (1.0 - T1);
           T2 = *Ids / Beta;
           T4 = n * model->BSIM2Vtm;
           T3 = *Ids / T4;
           if ((Vds > Vdsat) && here->pParam->BSIM2ai0 != 0.0)
           {   Ai = here->pParam->BSIM2ai0 + here->pParam->BSIM2aiB * Vbs;
               Bi = here->pParam->BSIM2bi0 + here->pParam->BSIM2biB * Vbs;
               T5 = Bi / (Vds - Vdsat);
               T5 = MIN(T5, 30.0);
           ...
           *gds = (T2 * dBeta_dVd + T3 * (here->pParam->BSIM2vofD * T4 - dVth_dVd
	        - here->pParam->BSIM2nD * Vgst / n) + Beta * model->BSIM2Vtm 
		* T0 * T1) * FR + *Ids * dFR_dVd;
           *gm = (T2 * dBeta_dVg + T3) * FR + *Ids * dFR_dVg;
           *gmb = (T2 * dBeta_dVb + T3 * (here->pParam->BSIM2vofB * T4 - dVth_dVb 
	        + here->pParam->BSIM2nB * Vgst / (n * T1s * T1s) * dT1s_dVb)) * FR
	        + *Ids * dFR_dVb;
           *Ids *= FR;
       }
   }
...
ChargeComputation:
    ...
    *gds = MAX(*gds,1.0e-20);
    ...
    if ((model->BSIM2channelChargePartitionFlag > 1)
	 || ((!ChargeComputationNeeded) &&
	 (model->BSIM2channelChargePartitionFlag > -5)))
    {  
        *qg  = 0.0;
        *qd = 0.0;
        *qb = 0.0;
    ...
    else
    {
       ...
       {   Vbseff = here->pParam->BSIM2phi - Phisb;
	   dVbseff_dVb = -dPhisb_dVb;
       }
       Arg1 = Vgs - Vbseff - here->pParam->BSIM2vfb;
       Arg2 = Arg1 - Vgst;
	  ...
	  *cbs = -*cgs;
       ...
       {  if (Vgst < here->pParam->BSIM2vghigh)
	  {    Uvert = 1.0 + Vgst * (Ua + Vgst * Ub);
		    ...
		    * (here->pParam->BSIM2uaB + Vgst * here->pParam->BSIM2ubB);
               ...
               dVc_dVb = -T8 * dVth_dVb + here->pParam->BSIM2u1B * Vgst * Inv_Aa 
	               * Inv_Uvert - Vc * Inv_Aa * dAa_dVb - T9 * dUvert_dVb;
...
finished:       /* returning Values to Calling Routine */
    valuetypeflag = (int) model->BSIM2channelChargePartitionFlag;

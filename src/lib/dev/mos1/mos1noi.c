    ...
    double *OnDens;
{
	...
	".rs",              /* noise due to rs */        /* in MOS1defs.h */
	".id",              /* noise due to id */
	...
	for (inst=model->MOS1instances; inst != NULL; inst=inst->MOS1nextInstance) {
		...
		if (((NOISEAN*)ckt->CKTcurJob)->NStpsSm != 0) {
			...
			for (i=0; i < MOS1NSRCS; i++) {
			    (void)sprintf(name,"onoise.%s%s",inst->MOS1name,MOS1nNames[i]);
...
if (!data->namelist) return(E_NOMEM);
		(*(SPfrontEnd->IFnewUid))(ckt,
		    ...
		    case INT_NOIZ:
			for (i=0; i < MOS1NSRCS; i++) {
			    (void)sprintf(name,"onoise_total.%s%s",inst->MOS1name,MOS1nNames[i]);
...
if (!data->namelist) return(E_NOMEM);
		(*(SPfrontEnd->IFnewUid))(ckt,
			...
			(IFuid)NULL,name,UID_OTHER,(GENERIC **)NULL);
			    ...
			    (void)sprintf(name,"inoise_total.%s%s",inst->MOS1name,MOS1nNames[i]);

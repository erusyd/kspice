    ...
    double *OnDens;
{
	...
	".rs",              /* noise due to rs */        /* in MOS3defs.h */
	".id",              /* noise due to id */
	...
	for (inst=model->MOS3instances; inst != NULL; inst=inst->MOS3nextInstance) {
		...
		if (((NOISEAN*)ckt->CKTcurJob)->NStpsSm != 0) {
			...
			for (i=0; i < MOS3NSRCS; i++) {
			    (void)sprintf(name,"onoise.%s%s",inst->MOS3name,MOS3nNames[i]);
...
if (!data->namelist) return(E_NOMEM);
		(*(SPfrontEnd->IFnewUid))(ckt,
		    ...
		    case INT_NOIZ:
			for (i=0; i < MOS3NSRCS; i++) {
			    (void)sprintf(name,"onoise_total.%s%s",inst->MOS3name,MOS3nNames[i]);
...
if (!data->namelist) return(E_NOMEM);
		(*(SPfrontEnd->IFnewUid))(ckt,
			...
			(IFuid)NULL,name,UID_OTHER,(GENERIC **)NULL);
			    ...
			    (void)sprintf(name,"inoise_total.%s%s",inst->MOS3name,MOS3nNames[i]);

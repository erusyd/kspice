 ...
 */
void
BSIM1evaluate(vds,vbs,vgs,here,model,gmPointer,gdsPointer,gmbsPointer,
...

 {
    ...
    if((Uds  =  here->BSIM1uds + here->BSIM1udsB * vbs + 
        ...
        Uds  =  Uds / Leff;
        dUdsdVbs  =  here->BSIM1udsB / Leff;
        dUdsdVds  =  here->BSIM1udsD / Leff;
    }
    ...
    if( Eta <= 0 ) {   
        ...
        dEtadVds  =  here->BSIM1etaD;
        dEtadVbs  =  here->BSIM1etaB;
    }
    if( vbs < 0 ) {
        Vpb  =  Phi - vbs;
    } else {
    ...
    BetaVdd  =  (here->BSIM1betaVdd + here->BSIM1betaVddB * vbs);
    dBetaVdd_dVds  =  MAX( here->BSIM1betaVddD, 0.0); /* Modified */
    if( vds > Vdd ) {
        Beta0  =  BetaVdd + dBetaVdd_dVds * (vds - Vdd);
        dBeta0dVds  =  dBetaVdd_dVds;
        dBeta0dVbs  =  here->BSIM1betaVddB;
    } else {
        VddSquare  =  Vdd * Vdd;
        C1  =  ( -BetaVdd + Beta_Vds_0 + dBetaVdd_dVds * Vdd) / VddSquare;
        C2  =  2 * (BetaVdd - Beta_Vds_0) / Vdd - dBetaVdd_dVds;
        dBeta_Vds_0_dVbs  =  here->BSIM1betaZeroB;
        dBetaVdd_dVbs  =  here->BSIM1betaVddB;
 ...
 *  if( (N0 >=  200) || (Vgs_Vth < Vcut ) || (Vgs_Vth > (-0.5*Vcut))) { */
    ...
    NB  =  here->BSIM1subthSlopeB;
    ND  =  here->BSIM1subthSlopeD;
    ...
    Warg2  =  6.04965 * Vtsquare * here->BSIM1betaZero;
    Ilimit  =  4.5 * Vtsquare * here->BSIM1betaZero;
    Iexp = Warg2 * Wgs * Wds;

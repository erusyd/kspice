...
d_p.d3_pqr = 0.0;
    ...
    if((Ugs  =  here->BSIM1ugs + here->BSIM1ugsB * vbs) <= 0 ) {
        Ugs = 0;
	EqualDeriv(&d_Ugs,&d_zero);
    } else {
    EqualDeriv(&d_Ugs,&d_q);
    d_Ugs.value = Ugs;
    d_Ugs.d1_q = here->BSIM1ugsB;
    }
    if((Uds  =  here->BSIM1uds + here->BSIM1udsB * vbs + 
            here->BSIM1udsD*(vds-Vdd)) <= 0 ) {    
        Uds = 0.0;
	EqualDeriv(&d_Uds,&d_zero);
    } else {
        Leff  =  here->BSIM1l * 1.e6 - model->BSIM1deltaL; /* Leff in um */
	/*const*/
        Uds  =  Uds / Leff;
	/* Uds = (here->BSIM1uds + here->BSIM1udsB * vbs here->BSIM1udsD*
		(vds-Vdd))/Leff */
	EqualDeriv(&d_Uds,&d_r);
	d_Uds.value = Uds;
	d_Uds.d1_r = here->BSIM1udsD/Leff;
	d_Uds.d1_q = here->BSIM1udsB/Leff;
    }
    Eta  =  here->BSIM1eta + here->BSIM1etaB * vbs + here->BSIM1etaD * 
        (vds - Vdd);
	EqualDeriv(&d_Eta,&d_r);
	d_Eta.value = Eta;
	d_Eta.d1_r = here->BSIM1etaD;
	d_Eta.d1_q = here->BSIM1etaB;
    ...
    Beta_Vds_0  =  (here->BSIM1betaZero + here->BSIM1betaZeroB * vbs);
    EqualDeriv(&d_Beta_Vds_0,&d_q);
    d_Beta_Vds_0.value = Beta_Vds_0;
    d_Beta_Vds_0.d1_q = here->BSIM1betaZeroB;
    BVdd  =  (here->BSIM1betaVdd + here->BSIM1betaVddB * vbs);
    EqualDeriv(&d_BVdd,&d_q);
    d_BVdd.value = BVdd;
    d_BVdd.d1_q = here->BSIM1betaVddB;
    dBVdddVds = MAX( here->BSIM1betaVddD, 0.0);
...
SubthresholdComputation:
    ...
    N0  =  here->BSIM1subthSlope;/*const*/
    Vcut  =  - 40. * N0 * CONSTvt0 ;/*const*/
    if( (N0 >=  200) || (VgsVth < Vcut ) || (VgsVth > (-0.5*Vcut))) {
        goto ChargeComputation;
    }
    
    NB  =  here->BSIM1subthSlopeB;/*const*/
    ND  =  here->BSIM1subthSlopeD;/*const*/
    ...
    Warg2  =  6.04965 * Vtsquare * here->BSIM1betaZero;/*const*/
    Ilimit  =  4.5 * Vtsquare * here->BSIM1betaZero;/*const*/
    Iexp = Warg2 * Wgs * Wds;
...
/*    if(DrCur < 0.0) DrCur = 0.0;
    ...
    */
    ...
    WLCox = model->BSIM1Cox * 
        (here->BSIM1l - model->BSIM1deltaL * 1.e-6) * 
        (here->BSIM1w - model->BSIM1deltaW * 1.e-6) * 1.e4;   /* F */
    ...
    if( model->BSIM1channelChargePartitionFlag >= 1 ) {
...
here->qg_x2 = 0.5*model->BSIM1type*here->qg_x2;
here->qg_y2 = 0.5*model->BSIM1type*here->qg_y2;
here->qg_z2 = 0.5*model->BSIM1type*here->qg_z2;
here->qg_xy = model->BSIM1type*here->qg_xy;
here->qg_yz = model->BSIM1type*here->qg_yz;
here->qg_xz = model->BSIM1type*here->qg_xz;
...
here->qb_x2 = 0.5*model->BSIM1type*here->qb_x2;
here->qb_y2 = 0.5*model->BSIM1type*here->qb_y2;
here->qb_z2 = 0.5*model->BSIM1type*here->qb_z2;
here->qb_xy = model->BSIM1type*here->qb_xy;
here->qb_yz = model->BSIM1type*here->qb_yz;
here->qb_xz = model->BSIM1type*here->qb_xz;
...
here->qd_x2 = 0.5*model->BSIM1type*here->qd_x2;
here->qd_y2 = 0.5*model->BSIM1type*here->qd_y2;
here->qd_z2 = 0.5*model->BSIM1type*here->qd_z2;
here->qd_xy = model->BSIM1type*here->qd_xy;
here->qd_yz = model->BSIM1type*here->qd_yz;
here->qd_xz = model->BSIM1type*here->qd_xz;
...
here->DrC_x2 = 0.5*model->BSIM1type*here->DrC_x2;
here->DrC_y2 = 0.5*model->BSIM1type*here->DrC_y2;
here->DrC_z2 = 0.5*model->BSIM1type*here->DrC_z2;
here->DrC_xy = model->BSIM1type*here->DrC_xy;
here->DrC_yz = model->BSIM1type*here->DrC_yz;
here->DrC_xz = model->BSIM1type*here->DrC_xz;
...
here->gbs2 = model->BSIM1type*here->gbs2;
here->gbd2 = model->BSIM1type*here->gbd2;

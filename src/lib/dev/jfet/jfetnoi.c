    ...
    double *OnDens;
{
	...
	".rd",              /* noise due to rd */        /* consistent with the index definitions */
	".rs",              /* noise due to rs */        /* in JFETdefs.h */
	".id",              /* noise due to id */
	...
	for (inst=model->JFETinstances; inst != NULL; inst=inst->JFETnextInstance) {
		...
		if (((NOISEAN*)ckt->CKTcurJob)->NStpsSm != 0) {
			...
			for (i=0; i < JFETNSRCS; i++) {
			    (void)sprintf(name,"onoise.%s%s",inst->JFETname,JFETnNames[i]);
...
if (!data->namelist) return(E_NOMEM);
		(*(SPfrontEnd->IFnewUid))(ckt,
		    ...
		    case INT_NOIZ:
			for (i=0; i < JFETNSRCS; i++) {
			    (void)sprintf(name,"onoise_total.%s%s",inst->JFETname,JFETnNames[i]);
...
if (!data->namelist) return(E_NOMEM);
		(*(SPfrontEnd->IFnewUid))(ckt,
			...
			(IFuid)NULL,name,UID_OTHER,(GENERIC **)NULL);
			    ...
			    (void)sprintf(name,"inoise_total.%s%s",inst->JFETname,JFETnNames[i]);

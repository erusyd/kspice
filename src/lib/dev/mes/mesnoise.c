    ...
    double *OnDens;
{
	...
	".rs",              /* noise due to rs */        /* in MESdefs.h */
	".id",              /* noise due to id */
	...
	for (inst=model->MESinstances; inst != NULL; inst=inst->MESnextInstance) {
		...
		if (((NOISEAN*)ckt->CKTcurJob)->NStpsSm != 0) {
			...
			for (i=0; i < MESNSRCS; i++) {
			    (void)sprintf(name,"onoise.%s%s",inst->MESname,MESnNames[i]);
...
if (!data->namelist) return(E_NOMEM);
		(*(SPfrontEnd->IFnewUid))(ckt,
		    ...
		    case INT_NOIZ:
			for (i=0; i < MESNSRCS; i++) {
			    (void)sprintf(name,"onoise_total.%s%s",inst->MESname,MESnNames[i]);
...
if (!data->namelist) return(E_NOMEM);
		(*(SPfrontEnd->IFnewUid))(ckt,
			...
			(IFuid)NULL,name,UID_OTHER,(GENERIC **)NULL);
			    ...
			    (void)sprintf(name,"inoise_total.%s%s",inst->MESname,MESnNames[i]);

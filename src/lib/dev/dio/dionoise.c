    ...
    double *OnDens;
{
	...
	".rs",              /* noise due to rs */        /* consistent with the index definitions */
	".id",              /* noise due to id */        /* in DIOdefs.h */
	...
	for (inst=model->DIOinstances; inst != NULL; inst=inst->DIOnextInstance) {
		...
		if (((NOISEAN*)ckt->CKTcurJob)->NStpsSm != 0) {
			...
			for (i=0; i < DIONSRCS; i++) {
			    (void)sprintf(name,"onoise.%s%s",inst->DIOname,DIOnNames[i]);
...
if (!data->namelist) return(E_NOMEM);
		(*(SPfrontEnd->IFnewUid))(ckt,
		    ...
		    case INT_NOIZ:
			for (i=0; i < DIONSRCS; i++) {
			    (void)sprintf(name,"onoise_total.%s%s",inst->DIOname,DIOnNames[i]);
...
if (!data->namelist) return(E_NOMEM);
		(*(SPfrontEnd->IFnewUid))(ckt,
			...
			(IFuid)NULL,name,UID_OTHER,(GENERIC **)NULL);
			    ...
			    (void)sprintf(name,"inoise_total.%s%s",inst->DIOname,DIOnNames[i]);

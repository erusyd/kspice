    ...
    double *OnDens;
{
	...
	".rs",              /* noise due to rs */        /* in MOS2defs.h */
	".id",              /* noise due to id */
	...
	for (inst=model->MOS2instances; inst != NULL; inst=inst->MOS2nextInstance) {
		...
		if (((NOISEAN*)ckt->CKTcurJob)->NStpsSm != 0) {
			...
			for (i=0; i < MOS2NSRCS; i++) {
			    (void)sprintf(name,"onoise.%s%s",inst->MOS2name,MOS2nNames[i]);
...
if (!data->namelist) return(E_NOMEM);
		(*(SPfrontEnd->IFnewUid))(ckt,
		    ...
		    case INT_NOIZ:
			for (i=0; i < MOS2NSRCS; i++) {
			    (void)sprintf(name,"onoise_total.%s%s",inst->MOS2name,MOS2nNames[i]);
...
if (!data->namelist) return(E_NOMEM);
		(*(SPfrontEnd->IFnewUid))(ckt,
			...
			(IFuid)NULL,name,UID_OTHER,(GENERIC **)NULL);
			    ...
			    (void)sprintf(name,"inoise_total.%s%s",inst->MOS2name,MOS2nNames[i]);

    ...
    int restart;    /* forced restart flag */
{
	...
	ckt->CKTtimeListSize = (int) floor(0.5 +
				ckt->CKTfinalTime/ckt->CKTstep);

	if (ckt->CKTtimePoints != NULL) FREE(ckt->CKTtimePoints);
	if (ckt->CKTdeltaList != NULL) FREE(ckt->CKTdeltaList);
	ckt->CKTtimePoints = (double *) MALLOC( sizeof(double) *
						ckt->CKTtimeListSize);
	ckt->CKTdeltaList = (double *) MALLOC( sizeof(double) *
						ckt->CKTtimeListSize);
...
nextTime:
    ...
    /* begin LTRA code addition */
    ckt->CKTtimeIndex++;
    if (ckt->CKTtimeIndex >= ckt->CKTtimeListSize) { /* need more space */
	...
	ckt->CKTdeltaList = (double *) REALLOC( (char *)
	     ckt->CKTdeltaList, sizeof(double) * ckt->CKTtimeListSize);
    }

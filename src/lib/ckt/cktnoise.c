    ...
    Ndata *data;
{
	...
	case N_DENS:
	    ...
	    (*(SPfrontEnd->IFnewUid))(ckt, &(data->namelist[data->numPlots++]),
		    (IFuid)NULL,"onoise_spectrum",UID_OTHER,(GENERIC **)NULL);

	    data->namelist = (IFuid *)trealloc((char *)data->namelist,
		    (data->numPlots + 1)*sizeof(IFuid));

	    (*(SPfrontEnd->IFnewUid))(ckt, &(data->namelist[data->numPlots++]),
		    (IFuid)NULL,"inoise_spectrum",UID_OTHER,(GENERIC **)NULL);
	...
	case INT_NOIZ:
	    ...
	    data->namelist = (IFuid *)trealloc((char *)data->namelist,
		    (data->numPlots + 1)*sizeof(IFuid));
	    (*(SPfrontEnd->IFnewUid))(ckt, &(data->namelist[data->numPlots++]),
		(IFuid)NULL,"onoise_total",UID_OTHER,(GENERIC **)NULL);

	    data->namelist = (IFuid *)trealloc((char *)data->namelist,
		    (data->numPlots + 1)*sizeof(IFuid));
	    (*(SPfrontEnd->IFnewUid))(ckt, &(data->namelist[data->numPlots++]),
		(IFuid)NULL,"inoise_total",UID_OTHER,(GENERIC **)NULL);
	    /* we've added two more plots */
	...
	if (data->NplotPtr) FREE(data->NplotPtr);
        break;
